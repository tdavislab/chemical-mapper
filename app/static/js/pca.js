class PCA{
    ...
    draw_PCA(points_dict){
        ...
        let pca_svg = d3.select("#PCA-panel").select(".block_body-inner").append("svg")
            ...
            .attr("width", width)
            .attr("height", height);
        pca_svg.append("g").attr("id","pca_axis_group");
        ...
        // x-axis
        d3.select("#pca_axis_group").append("g") 
...
function draw_KNN(points)
{
    ...
    let knn_svg = d3.select("#KNN-panel").select(".block_body-inner").append("svg")
        ...
        .attr("width", width)
        .attr("height", height);
    knn_svg.append("g").attr("id","knn_axis_group");
    ...
    // x-axis
    d3.select("#knn_axis_group").append("g") 
    .call(d3.axisBottom(xScale).ticks(5))
    .classed("axis_line", true)
    .attr("transform", "translate(0,"+(height-margin.bottom)+")");
